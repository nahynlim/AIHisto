...
def mainbody(build_model, csv, entries, outpth=None, clnum=None, progress_bar=None):
    ...
    # Ensure numeric parameters are the right types
    if clnum is not None:
        try:
            clnum = int(clnum)
        except Exception:
            clnum = int(str(clnum).strip())
    if build_model:
        ...
        model_dir = os.path.join(outpth, modelname)
        os.makedirs(model_dir, exist_ok=True)
        if os.path.exists(os.path.join(model_dir, modelname + '.pickle')):
            f = open(os.path.join(model_dir, modelname + '_' + realtimedate + '.pickle'), 'wb')
        else:
            f = open(os.path.join(model_dir, modelname + '.pickle'), 'wb')
